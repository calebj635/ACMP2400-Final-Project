...
#STAGE 1
#Configure provider, remote state store, and azure container registry
...
#STAGE 2
#Configure azure container instance that will run in cloud. We must autoapprove bc of the non-interactivity.
elif [[ "$TF_STAGE" == "stage2" ]]; then
  terraform -chdir=${INPUT_TF_STAGE} init -beckend-config="key=${INPUT_STATE_KEY}.tfstate"
  terraform -chdir=${INPUT_TF_STAGE} apply -auto-approve -var="ARM_CLIENT_ID=${INPUT_ARM_CLIENT_ID}" -var="ARM_CLIENT_SECRET=${INPUT_ARM_CLIENT_SECRET}" -var="DJANGO_SECRET_KEY_PROD=${DJANGO_SECRET_KEY_PROD}"
